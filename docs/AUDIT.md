...
| Beacon secret | random 32 bytes per viewer (`crypto.js` `randomBytes(32)` via `helpsession.js`) | memory only; ends on check-in, stop-sharing, app lock, revocation of that one viewer, or process death (`helpsession.js:100-120`, `helpsession.js:157-163`) | any of the above events, per viewer independently | nothing to miss; it was never written to disk |
